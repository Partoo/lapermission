...
Partoo is a web design agency in Antwerp, Belgium. You'll find an overview of all
...
'providers' => [
    // ...
    Partoo\Permission\PermissionServiceProvider::class,
];
```
...
```bash
php artisan vendor:publish --provider="Partoo\Permission\PermissionServiceProvider" --tag="migrations"
```
...
```bash
php artisan vendor:publish --provider="Partoo\Permission\PermissionServiceProvider" --tag="config"
```
...
return [
    ...
    'models' => [
         ...
         * `Partoo\Permission\Contracts\Permission` contract.
         */

        'permission' => Partoo\Permission\Models\Permission::class,
         ...
         * `Partoo\Permission\Contracts\Role` contract.
         */

        'role' => Partoo\Permission\Models\Role::class,
...
$app->routeMiddleware([
    ...
    'permission' => Partoo\Permission\Middlewares\PermissionMiddleware::class,
    'role'       => Partoo\Permission\Middlewares\RoleMiddleware::class,
]);
```
...
$app->register(Partoo\Permission\PermissionServiceProvider::class);
```
...
First, add the `Partoo\Permission\Traits\HasRoles` trait to your `User` model(s):

```php
use Illuminate\Foundation\Auth\User as Authenticatable;
use Partoo\Permission\Traits\HasRoles;
...
```php
use Partoo\Permission\Models\Role;
use Partoo\Permission\Models\Permission;
...
The `role` scope can accept a string, a `\Partoo\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
The scope can accept a string, a `\Partoo\Permission\Models\Permission` object or an `\Illuminate\Support\Collection` object.
...
The `assignRole`, `hasRole`, `hasAnyRole`, `hasAllRoles`  and `removeRole` functions can accept a
 string, a `\Partoo\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
string or a `Partoo\Permission\Models\Permission` object.
...
All these responses are collections of `Partoo\Permission\Models\Permission` objects.
...
protected $routeMiddleware = [
    ...
    'role' => \Partoo\Permission\Middlewares\RoleMiddleware::class,
    'permission' => \Partoo\Permission\Middlewares\PermissionMiddleware::class,
    'role_or_permission' => \Partoo\Permission\Middlewares\RoleOrPermissionMiddleware::class,
];
```
...
{
    if ($exception instanceof \Partoo\Permission\Exceptions\UnauthorizedException) {
        // Code here ...
    }
...
```php
    public function setUp()
    {
        ...
        $this->app->make(\Partoo\Permission\PermissionRegistrar::class)->registerPermissions();
    }
```
...
class RolesAndPermissionsSeeder extends Seeder
{
    public function run()
    {
        ...
        app()[\Partoo\Permission\PermissionRegistrar::class]->forgetCachedPermissions();
...
- Your `Role` model needs to extend the `Partoo\Permission\Models\Role` model
- Your `Permission` model needs to extend the `Partoo\Permission\Models\Permission` model

If you need to REPLACE the existing `Role` or `Permission` models you need to keep the
following things in mind:

- Your `Role` model needs to implement the `Partoo\Permission\Contracts\Role` contract
- Your `Permission` model needs to implement the `Partoo\Permission\Contracts\Permission` contract
...
```php
app()->make(\Partoo\Permission\PermissionRegistrar::class)->forgetCachedPermissions();
```
...
Our address is: Partoo, Samberstraat 69D, 2060 Antwerp, Belgium.
...
Partoo is a web design agency based in Antwerp, Belgium. You'll find an overview of all our open source projects [on our website](https://spatie.be/opensource).
